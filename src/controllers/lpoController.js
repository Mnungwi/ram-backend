...
// ── Sync tables ONCE on startup ───────────────────────────────────────────────
// let tablesReady = false;
// const ensureTables = async () => {
//   if (tablesReady) return;
//   await LocalPurchaseOrder.sync({ alter: true });
//   await LPOItem.sync({ alter: true });
//   await LPOComment.sync({ alter: true });
//   tablesReady = true;
// };
...
exports.listLPOs = async (req, res, next) => {
  try {
    ...
    if (search) where[Op.or] = [{ lpoNo: { [Op.like]: `%${search}%` } }];
...
exports.createLPO = async (req, res, next) => {
  try {
    ...
    await LPOItem.bulkCreate(
      itemsWithAmount.map((item) => ({ ...item, lpoId: lpo.id })),
    );
    await audit({
      userId: req.userId,
      action: "create_lpo",
    ...
    const full = await LocalPurchaseOrder.findByPk(lpo.id, {
      include: LPO_INCLUDES,
    });
  ...
  try {
    ...
    const itemsWithAmount = rn.items.map((item, idx) => {
      ...
      return {
        ...
        unitPrice: parseFloat(prices[item.id] || item.unitPrice || 0),
        amount:
          parseFloat(item.quantityOrdered) *
          parseFloat(prices[item.id] || item.unitPrice || 0),
        quantityReceived: 0,
    ...
    return successResponse(
      res,
      { lpo: full },
      `LPO created from ${rn.requisitionNo}`,
      201,
    );
  } catch (err) {
    next(err);
...
exports.updateLPO = async (req, res, next) => {
  try {
    ...
    if (!lpo) return errorResponse(res, "LPO not found", 404);
    if (lpo.status !== "draft")
      return errorResponse(res, "Only draft LPOs can be edited", 400);
...
exports.submitLPO = async (req, res, next) => {
  try {
    ...
    if (!lpo) return errorResponse(res, "LPO not found", 404);
    if (lpo.status !== "draft")
      return errorResponse(res, "Only draft LPOs can be submitted", 400);
    await lpo.update({ status: "submitted", submittedAt: new Date() });
    await LPOComment.create({
      lpoId: lpo.id,
      userId: req.userId,
      step: "submitted",
      comment: req.body.comment || "LPO submitted for approval.",
    });
    await audit({
      userId: req.userId,
      action: "submit_lpo",
...
exports.approveLPO = async (req, res, next) => {
  try {
    ...
    if (!lpo) return errorResponse(res, "LPO not found", 404);
    if (lpo.status !== "submitted")
      return errorResponse(res, "Only submitted LPOs can be approved", 400);
    await lpo.update({
      status: "approved",
      approvedAt: new Date(),
    ...
    await LPOComment.create({
      ...
      userId: req.userId,
      step: "approved",
      comment: req.body.comment || "LPO approved.",
    });
    await audit({
      userId: req.userId,
      action: "approve_lpo",
...
exports.sendLPO = async (req, res, next) => {
  try {
    ...
    if (lpo.status !== "approved")
      return errorResponse(res, "Only approved LPOs can be sent", 400);
    await lpo.update({ status: "sent", sentAt: new Date() });
    await LPOComment.create({
      lpoId: lpo.id,
      userId: req.userId,
      step: "sent",
      comment: req.body.comment || "LPO sent to supplier.",
    });
    await audit({
      userId: req.userId,
      action: "send_lpo",
    ...
    return successResponse(res, { lpo }, "LPO sent");
  } catch (err) {
    next(err);
  }
};
...
//     });
//     if (!lpo) return errorResponse(res, "LPO not found", 404);
//     if (!["sent", "partial"].includes(lpo.status))
//       return errorResponse(res, "LPO must be sent before receiving", 400);
...
//       (i) => parseFloat(i.quantityReceived) >= parseFloat(i.quantity),
//     );
...
//       comment:
//         comment ||
//         (allReceived ? "All items received." : "Partial delivery received."),
//     });
//     await audit({
//       userId: req.userId,
//       action: "receive_lpo",
...
//       const { Op } = require("sequelize");
//       const receivedLpoItems = await LPOItem.findAll({
//         where: { lpoId: lpo.id },
//       });

//       for (const item of receivedLpoItems) {
//         const qty = parseFloat(item.quantityReceived) || 0;
//         if (qty <= 0) continue;

//         let storeItem = await ProjectStoreItem.findOne({
//           where: {
//             projectId: lpo.projectId,
//             [Op.or]: [
...
//         await storeItem.update({ quantity: newQty });
...
//         console.log(
//           `Store updated: ${item.description} +${qty} → balance: ${newQty}`,
//         );
//       }
//     } catch (storeErr) {
//       console.error("Store update error:", storeErr.message);
//     }
...
//       allReceived
//         ? "All items received & store updated"
//         : "Partial delivery recorded & store updated",
//     );
//   } catch (err) {
//     next(err);
//   }
// };
// Replace the entire receiveLPO function in lpoController.js
...
exports.receiveLPO = async (req, res, next) => {
  try {
    ...
    // Fetch updated items WITH productId explicitly
    const updatedItems = await LPOItem.findAll({
      where: { lpoId: lpo.id },
      attributes: ['id', 'description', 'unit', 'quantity', 'quantityReceived', 'unitPrice', 'productId', 'requisitionItemId'],
    });
    ...
    // ── Auto-update Project Store ──────────────────────────────
    try {
      const { ProjectStoreItem, StoreTransaction } = require('../models/store.model');
      const { Op } = require('sequelize');
      ...
      for (const item of updatedItems) {
        ...
        const orConditions = [];
        if (item.productId) orConditions.push({ productId: item.productId });
        orConditions.push({ description: item.description });
        ...
        } else if (item.productId && !storeItem.productId) {
          // Update productId if missing
          await storeItem.update({ productId: item.productId });
        }
        ...
        console.log(`Store updated: ${item.description} (${item.productId || 'no-product'}) +${qty} → balance: ${newQty}`);
      }
    } catch (storeErr) {
      console.error('Store update error:', storeErr.message);
    }

    // ── Update Requisition status to completed if linked ────────
    try {
      if (lpo.requisitionId) {
        const { Requisition } = require('../models/requisition.model');
        const rn = await Requisition.findByPk(lpo.requisitionId);
        if (rn && rn.status === 'issued') {
          await rn.update({ status: 'completed' });
          console.log(`Requisition ${rn.requisitionNo} marked as completed`);
        }
      }
    } catch (rnErr) {
      console.error('Requisition update error:', rnErr.message);
    }

    return successResponse(
      res,
      { lpo, status: newStatus },
      allReceived ? 'All items received & store updated' : 'Partial delivery recorded & store updated',
    );
  } catch (err) {
    next(err);
  }
...
exports.cancelLPO = async (req, res, next) => {
  try {
    ...
    if (!lpo) return errorResponse(res, "LPO not found", 404);
    if (["received", "paid", "cancelled"].includes(lpo.status))
      return errorResponse(res, "Cannot cancel this LPO", 400);
    await lpo.update({ status: "cancelled" });
    await LPOComment.create({
      lpoId: lpo.id,
      userId: req.userId,
      step: "cancelled",
      comment: req.body.reason || "LPO cancelled.",
    });
...
exports.deleteLPO = async (req, res, next) => {
  try {
    ...
    if (!lpo) return errorResponse(res, "LPO not found", 404);
    if (!["draft", "cancelled"].includes(lpo.status))
      return errorResponse(
        res,
        "Only draft or cancelled LPOs can be deleted",
        400,
      );
    await LPOItem.destroy({ where: { lpoId: lpo.id } });
